...
> **备份间隔的两处定夺，2026-09-10。**
>
> **① 备份页报的是「用户挑的那个档位名」，不是天数。**
>
> 说法的单一出处是 `autoBackupIntervalDays.options`，备份页与设置页
> 都从那儿取，所以两处**已经一致**（都说「每周」）。
>
> **「每月」是 30 天的有意近似**：排期实打实按 `Duration(days: 30)`
> 走（`BackupService.autoBackupIfDue`），只有**说法**跟着用户挑的
> 那个词。两个方向都别再动它：
>
> - 不要再去「统一措辞」—— 已经一致了；
> - **不要把 30 天改成真的日历月** —— 近似的是说法，行为是有意的，
>   而改它会连 `decode` 的白名单 `[1, 7, 30]` 一起搅进去。
>
> <details><summary>这一条 12:47 写下时是反的，留个记录</summary>
>
> 当时写的是「不统一，因为备份页在陈述当前生效的值、跟着
> `intervalDays` 走」，五分钟后的提交就把它改成读 `options` 了 ——
> 文档的前提成了一个不存在的现状。理由也反了：旧那句
> 「每 30 天一次」对**行为**是准的，不准的是**标签**「每月」；
> 真正扎眼的是「每 1 天一次」。
>
> 记下来是因为这段文字存在的唯一目的是「别再重新讨论一遍」，
> 而**一段自相矛盾的定夺比没有定夺更糟** —— 它带着「这事已经定了」
> 的语气，却会把下一个人推向重新讨论，或者推向一个没人要过的
> 行为变更。
>
> </details>
>
> **② 自动备份关掉之后，「备份间隔」那排档位仍然可点 —— 不灰掉。**
>
> 看起来该套用「点了没反应的开关比没有更糟」那条，其实不是同一族：
> 那条说的是**点了什么也不发生**（`toggle` 没实现时的那句灰字）。
> 而这几个档位**点了就写进库、值也留着**，开关一开当场生效 ——
> 它不是没反应，是「现在还用不上」。
>
> 更要紧的是：**同一个形状在「免打扰」那儿已经有三处**
> （`quietHoursEnabled` 关着时，开始 / 结束 / 时段内照样可点）。
> 只灰掉备份这一处，应用内部就不一致了；四处一起改则要给注册表加一个
> 「这项依赖那项」的概念（`dependsOn` + 禁用态渲染 + 用例），
> 那是一次**设置页的能力扩展**，不是收尾时顺手做的事 —— 排 M5。
>
> 记在这里是为了**别再重新讨论一遍**：下次有人觉得该灰掉，
> 要先回答「免打扰那三处怎么办」。
