...
2026-09-10 撞到一个：`TaskDraft.needsDateForRecurrence` **零个调用方**，
而另一处的注释引它来论证「这一支实际到不了」。真正维持那条约束的，
是别处**各写了一遍的同一个表达式**。
...
| 做法 | 结果 |
|---|---|
| 删掉那个 getter | 不一致消失了 —— **而事实继续裸奔**，注释还得改成指某一行代码 |
| **让维持它的那一处去读它** | 名字与事实之间那根线真的存在了，注释指的东西成了真的 |

> **删名字是让事实继续裸奔，接上线才是让名字重新变成保证。**

判据：那个名字**说的那件事**还成立吗？成立就接线，不成立才删。
（评审 2026-09-10 归纳。）
