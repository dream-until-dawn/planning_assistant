...
| 取消父任务完成 | **只收回「让它变成已完成的那一批」阶段**，之前就勾好的留着 |
| 勾/取消某个阶段 | 父任务状态按 §4 的表重新投影 —— **全部勾完才是 `done`** |

「那一批」的判据是**完成时刻等于任务的完成时刻**。这不是靠时间戳猜，是同一次操作的两半：
标完成时用同一个 `now` 写任务与它补上的阶段；逐个勾满时任务的完成时刻取自**最后那一步**
（`projectStagesOntoTask`）。于是两种来法都认得出来 ——
显式标完成收回级联补的全部，逐个勾满收回最后勾的那一个。

收回之后父任务的状态由**剩下的阶段推出来**，不强行落到 `pending`：
取消一条「1 步做完、1 步跳过、1 步待办」的任务，它回到 `inProgress`，那正是它当时的样子。
...
#### 这条规则被推翻过两次，两次的理由都记下来

**初版**：取消父任务完成时**不**回滚任何阶段，理由是
「破坏用户已记录的阶段进度比留下不一致更糟」，并注明「这条不对称是刻意的」。
它的问题在「阶段全勾完推出 `done`」那种来法下：不回滚就留下
「存的是 `pending`、推出来还是 `done`」的自相矛盾，取消完成等于什么也没发生。

**第二版**：回滚**所有** `done` 的阶段，论证是「那份进度在标完成的那一下就已经被盖掉了」。
**这句话对已经 `done` 的阶段是假的** —— 级联刻意跳过它们（`skipped` 与 `done` 都跳过），
一次都没碰过。于是这个序列会丢数据：

```
① 用户自己把第 0 步勾成 done，第 1 步明确跳过，第 2 步没做
② 用户标完成父任务 → 级联只把第 2 步 pending→done，第 0 步原样不动
③ 用户取消完成     → 第 0 步被回滚成 pending
```

第 0 步正是「用户已记录的阶段进度」，一字不差。而且它与同一版里对 `skipped` 的处理自相矛盾：
「这一步跳过」保得住，「这一步做完了」保不住，两者都是用户亲手记的。

> **教训：发现旧规则的一个反例，不等于可以换上另一个有反例的规则。**
> 两次都是把「全不回滚」与「全回滚」当成仅有的两个选项，
> 而正确答案在中间 —— 只回滚这次操作自己写下的那些。

规则改了两次，锁它的测试（L-05 / L-06）每次都是**改写而不是删掉** ——
删掉的话，将来谁把它改回去，没有任何东西会红。

#### 撤销还原的是阶段快照

「勾了一半 → 标完成 → 点撤销」这一串由撤销负责，而撤销**不是发一条反向的状态命令**：
反向命令走的是同一套级联，它不知道哪些阶段本来就勾着。撤销捕获点击那一刻的阶段快照，
按它原样写回，父任务的状态随后跟着阶段走。

快照会过期（提示挂着的几秒里用户可以在别处改某一步），所以**改动任一阶段时把提示撤下**
（`toggleStageDone`）—— 能撤销的窗口短一点，好过撤销把新改动盖掉。
