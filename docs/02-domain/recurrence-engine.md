...
| 全天任务 | 时长**必须是 1440 的整数倍**才产生 `end`，否则忽略（见下） |

**全天任务的结束只能是「整天」**：「全天」与「几点结束」是矛盾的，
所以半天的时长在全天任务上被无声忽略。但**跨几天的全天任务是有结束的**
—— 一次三天的休假结束在第三天。一律不给 `end` 的话，它在日历与甘特上
只剩第一天，那看起来像「数据只存了一天」，不像「刻意不表示时刻」。
整天数的 `end` 落在午夜，与全天 `start` 用的是同一个**占位午夜**
（`LocalWallTime.allDay`）—— 它表示的是哪一天，不是「零点」这个时刻。

**谁来提供 `durationMinutes`**：`occurrence_expansion._durationOf()`，
由任务自己的 `planDate`/`startMinute` → `endDate`/`endMinute` 折算。
这一处一度是空的，见[测试策略 §1.9](../05-engineering/testing-strategy.md#19-两侧都测到了中间那根线没人接)。
...
> **R-10…R-13 一度是「引擎会算、用户造不出来」的四条。** 编辑器的重复区
> 没有 `BYMONTHDAY` / 序号 `BYDAY` 的控件，`RecurrenceDraft` 里连字段都没有，
> 而这四条黄金用例一直是绿的 —— 它们证明的是引擎，不是这条能力到得了用户手上。
> 控件补上之后，锚在需求上的守卫见 `test/application/fr_task_03_shapes_test.dart`。
...
>
> **R-52 也补上了**（`test/presentation/stage_offset_follows_test.dart`）。
> 它验的其实是「什么都没发生」：§4.1 选相对偏移的好处之一就是
> 「整体挪任务时阶段自动跟随」—— 那不需要任何代码，
> 只需要**没有人去动那个偏移**。所以那条用例盯的是编辑器改开始时刻时
> 不去重算偏移，另有一条对照组钉住 §4.1 的另一半
> （改结束日期**不**缩放阶段时长，那是刻意的）。
>
> 写它的时候撞出一个真缺陷，见下。
