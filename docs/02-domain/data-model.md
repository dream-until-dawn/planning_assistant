...
| 守卫 | 作用域 | 它看不见什么 |
|---|---|---|
| `shared_span_test` | **到 provider 为止** —— 三个视图的 provider 从同一容器读，期望值手写 | painter 直接读 `row.endDate` 画东西 |
| `layer_dependency_test` 里那条「视图侧不得直接读原始结束时刻」 | `lib/features/views/**` 的源码 | `final x = row; x.endDate` 这类换了名字的写法（它是**名字启发式**，不是类型分析，注释里写明了） |

第二条是评审提的：第一条守的是「三个视图**目前**都走 provider」，
而不是「视图侧不许自己算」。两者缺一，
末阶段超出 `endDate` 的任务就可能在甘特与时间轴上有两个长度。
## 5. 同步信封（每张可同步表都有）
...
- 导入走的是原始 SQL（`customStatement`），drift **认不出**这种语句动了哪张表，
  所以导入结束前必须自己 `notifyUpdates` 一遍。少了这一步，数据层完全正确
  而**界面不刷新**：恢复成功、提示也弹了，列表却停在导入前的样子。
  `export_import_test` 里有一条挂着真订阅的用例盯着它。
...
一份备份**就是一份导出 JSON**，落在应用文档目录的 `backups/` 下。
判断全在 `BackupService` 里，落盘在 `BackupStore` 后面（`platform/storage/`）。

| 事项 | 定法 | 为什么 |
|---|---|---|
| 文件名 | `backup-<UTC ISO，冒号换成 `-`>.json` | 冒号在部分文件系统上非法；ISO 的其余部分保证**字典序 = 时间序** |
| 列表顺序 | 按创建时间**倒序**，由 `BackupStore.list()` 保证 | 保留策略要删「最旧的几份」、界面要「最新的在上面」，两处各排一次迟早排反一个 |
| 保留份数 | `data.autoBackupKeepCount`（隐藏，默认 5），超出的从最旧的删起 | |
| 自动备份判据 | **距最近一份文件多久**，不是存下来的时间戳 | 存时间戳就多一份可能与文件对不上的状态：用户把备份删光之后那个戳还说「刚备过」，于是再也不自动备份 |
| 触发时机 | **启动时问一次**（`AutoBackupScope`），且要等配置真读出来 | 备份是天级的事，挂生命周期只会让它被问上几百遍；而配置没读出来时解析层回落到**默认值**（开着），于是一个关掉了自动备份的用户照样会被备一份 |
| 恢复 | 整库替换（`ExportService.import` 的语义），**先存一份当前状态、再问一句、才动手** | 不做增量合并：合并需要冲突策略，而 V1 没有。留副本那一条见下 |
| 同一秒的两次备份 | 第二份的文件名缀一个序号，**不互相覆盖** | 从前只意味着少一份内容几乎相同的备份；有了「恢复前留副本」之后，被覆盖的那一份可能正是某次恢复的退路 |

> **恢复之前先留一份，与迁移前留副本是同一条原则。**
>
> NFR-REL-02 与 §7 要求迁移前把库文件复制成 `pre-migration-v{n}.db`。
> 恢复是同一类操作 ——「整库、不可逆、替换」—— 而且比迁移更该留：
> 迁移是自动发生的、没有选择，恢复是**用户手点的、可能点错一份**。
> 自动备份默认 7 天一次，所以点错的代价最坏是一周的数据。
>
> 三处顺序有讲究：先**读**要恢复的那一份（名字不对就当场失败，不必白存），
> 再**存**当前状态（存不下来就**放弃恢复** —— 宁可不恢复，也不要在没有
> 退路的情况下换掉整库），最后才导入。这一份**不参与保留数清理**：
> 清理按「最旧的先删」动手，而待恢复的那一份与刚存下的这一份
> 恰恰都可能落在被删的一侧。

> **V1 的备份是应用内的。** 列得出、恢复得回来，但用户拿不到应用外面去 ——
> 分享面板 / SAF 要引新插件，那是一次技术栈决定（版本矩阵 + 探针 + ADR）。
> 这是写下来的取舍，不是没做完；见 [roadmap](../07-roadmap/roadmap.md) 的 M4 与 M5。
