...
> ### `Stage.status` 对重复任务是**死数据**
>
> 重复任务的阶段状态存在这张表里，`Stage.status` 那一列**没人读**
> （判据见 `stageStatusFor`）。它存在、可写，而且写错了不会有任何东西变红。
>
> 这与 `tasks.status` 对重复任务恒为 `pending`（§4.3）是同一个形状 ——
> 那一条是用**显式抛异常**守住的。这一条目前只守住了「读」：
> 编辑器对重复任务不再显示阶段勾选框，视图侧有一条 lint 盯着
> `stage.status` 不许直接读。**写这一侧还没有守卫**
> （导入、V3 同步、V4 的 Agent 都够得着），记在这里。
