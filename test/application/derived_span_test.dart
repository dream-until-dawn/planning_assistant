...
void main() {
  ...
  group('推不出来时不要瞎推', () {
    test('一个带时间的阶段都没有 → null', () {
      ...
      // 而不是把任务的起止清掉。
      //
      // ## 这一条钉的是**语义**，下面那条钉的是**一致性**（评审 O-2）
      //
      // 它断言的是「这一组输入得到 null」，**不经过 `canDeriveSpan`**。
      // 下面那条断言两个函数答案相同 —— 今天它恒真（推导第一句就调
      // `canDeriveSpan`），守的是将来有人把判据抄回去。
      //
      // **两条缺一不可，而且危险的是删错哪一条。** 把 `canDeriveSpan`
      // 改错了（比如改成「至少两个阶段有时间」），界面与推导会**一起**
      // 变，下面那条照旧全绿 —— 只有这一条会红。
      // 看起来更朴素的这一条，才是那时唯一会响的东西。
    ...
    test('**`canDeriveSpan` 与「返回 null」是同一个判据**', () {
      ...
      //
      // ## 作用域：它钉的是**一致性**，不是语义（评审 O-2）
      //
      // 今天它**恒真** —— `deriveSpanFromStages` 第一句就调
      // `canDeriveSpan`，所以它不可能红。这不是缺点：它守的是将来有人
      // 把判据抄回推导里（`if (timed.isEmpty) return null`），
      // 那时它是唯一会响的东西。
      //
      // 语义由上面那条「一个带时间的阶段都没有 → null」钉着。
      // **删哪条都留个洞**：判据本身被改错时只有那条会红，
      // 判据被抄成两份时只有这条会红。
      for (final stages in [
        <StageTiming>[],
        [_s(null)],
