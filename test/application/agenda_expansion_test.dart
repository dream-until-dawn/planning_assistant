...
void main() {
  ...
  group('重复任务：本次和下次', () {
    ...
    test('规则已经结束、只剩逾期的 → 那几条全在', () {
      ...
      // 9/5 那次**不在**，而它不是被闭区间的边界卡掉的：
      // 那一次的绝对时刻是 `2026-09-05T01:00Z`（本地 09:00 +08），
      // **严格晚于** `UNTIL` 的 `2026-09-05T00:00Z`。
      // 即使 UNTIL 按闭区间算也救不了它。
      //
      // 写期望时按本地日期数会多算一条 —— 这条注释就是那次算错留下的。
      // 评审用两条不共用的路径核过（绝对时刻域、以及代码实际走的
      // 墙钟域 `untilForExpansion`），两条给出同一个答案。
