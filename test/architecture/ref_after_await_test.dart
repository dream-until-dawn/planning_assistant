...
/// ## 它有一条隐性依赖，说出口了就写下来
///
/// `await g(); ref.invalidate(p);` **写在同一行**时这条守卫不报 ——
/// 它是按行推进的。而这个洞现在由**别的门禁**堵着：
/// `ci.yml` 里那句 `dart format --set-exit-if-changed lib test tool`
/// 会把两条语句拆到两行去（实测过）。
///
/// 所以**本条的完备性依赖格式门禁**。写下来是因为一条没说出口的依赖，
/// 在它被拿掉的那天不会有任何信号（同 §3.6 那条不变量的来历）。
/// 真要自己堵，判别符是分号：`await` 与 `ref` 之间隔着 `;` 就是新语句。
@TestOn('vm')
library;
