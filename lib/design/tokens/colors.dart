...
abstract final class BrandColors {
  ...
  ///
  /// [primaryGraphic] 对三个亮表面只有 **3.46 / 3.46 / 3.10** —— 图形够用
  /// （门槛 3.0），正文不够（门槛 4.5）。所以品牌色也要按 §2.4 那条规矩
  /// 拆成填充 / 图形 / 文字三级，语义色早就拆了，品牌色这里补齐。
  ///
  /// 前两个相同不是笔误：§2.2 起画布与卡片都是 `#FFFFFF`，分母是同一个
  /// （design-system §2.2 那条注也写着这件事）。
  ///
  /// ## 这三个数漂过一次
  ///
  /// 原来写的是 `3.35 / 3.46 / 3.08`，那是**改配色之前**的值
  /// （canvas 还是奶油白 `#FDFBF7`、sunken 还是 `#F5F1EA`）。
  /// 三个全过期了，而**没有任何东西会红** —— 门槛（≥3.0）由
  /// `contrast_test.dart` 的「primaryGraphic 对三个亮表面都 ≥ 3.0:1」
  /// 守着，它从**活的 token** 现算，所以配色一改它自动跟上；
  /// 而这段注释里的数字是**手抄的一份副本**，没人管。
  ///
  /// 记在这儿是因为它是 testing-strategy §1.17.1.1 的又一个实例：
  /// **写下来的数字若是别处也会算一遍的，它迟早与那一份分叉。**
  /// 文档里的对比度数字已经机检了（`5a4d3c8`），
  /// 代码注释里的**还没有** —— 见 §1.12.1。
  ///
