...
abstract final class BrandColors {
  ...
  ///
  /// [primaryGraphic] 对**所有**亮表面都过得了图形门槛（≥3:1）、
  /// 又**都够不到**正文门槛（4.5:1）—— 所以品牌色要按 §2.4 那条规矩
  /// 拆成填充 / 图形 / 文字三级，语义色早就拆了，品牌色这里补齐。
  ///
  /// 两端分别由 `contrast_test.dart` 的
  /// 「primaryGraphic 对**三个**亮表面都 ≥ 3.0:1，且都够不到 4.5」守着；
  /// **想知道离门槛还剩多少余量，看那边**（「把 primaryGraphic 的三个
  /// 实测值钉住」那条里写着数）。
  ///
  /// ## 这里一度抄着三个具体数字
  ///
  /// 写的是改配色之前的值（画布还是奶油白、sunken 还是暖灰）。
  /// 换配色之后**三个全过期，而没有任何东西会红** —— 门槛由测试从活
  /// token 现算，注释里的是手抄的副本。
  ///
  /// 更糟的在后面：评审复核时用了同一份旧值，算出来与注释一致、
  /// 判了「✓」，三个里只挑出一个。**一份不受检的副本不只是陈旧，
  /// 它会主动提供假的确证**（testing-strategy §1.12.1）。
  ///
  /// 所以数字搬进断言（那里过期就会红），这儿只留**主张**与指针：
  /// **散文里放主张，不放测量** —— 主张跨配色稳定，测量不稳定，
  /// 而脱离了生成机制的测量会从「信息」变成「诱饵」。评审提的。
  ///
  /// 值是**派生的不是拍的**：同色相同饱和下，用 `deriveGraphicColor`
  /// 以 ratio 4.5 对最暗亮表面（sunken）二分求解，取仍达标的**最大亮度**
  /// —— 也就是在合规前提下最接近品牌色的那一个。
  ///
  /// 它对三个亮表面都过正文门槛（4.5:1），由 `contrast_test.dart` 的
  /// 「正文文字 × 三个亮表面 ≥ 4.5:1」守着，实测值也在那边钉着。
  ///
  /// ## 这里原来写着「实测 5.12 / 4.95 / 4.55 ✅」
  ///
  /// 两处毛病，后一处更要紧：
  ///
  ///  · 三个数是**旧配色**下的（而且是 card / canvas / sunken 的顺序，
  ///    与句子里的顺序都对不上）；
  ///  · **那个 ✅ 背后什么也没有** —— `primaryText` 当时根本不在
  ///    「正文文字 × 三个亮表面」那张表里。这个 token 存在的全部理由
  ///    就是「品牌色作正文时用它」，而它的定义性质从来没人验过。
  ///
  /// 是顺着上面那处过期数字往下读时撞见的：
