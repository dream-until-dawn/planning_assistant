...
enum ViewKind {
  ...
  timeline('timeline', '时间轴', anchorsToDay: false),
  ...
  ///
  /// 决定加号带不带日期：日历与甘特都有一个用户正看着的聚焦日，
  /// 在那儿点加号，他说的是「这一天」——
  /// 不带的话，日历翻到 9/20 建出来的任务会落进「无日期」。
  ///
  /// **列表与时间轴不是**，两者都横跨所有日期，用户在那儿点加号
  /// 没有指向任何一天。一律盖上「今天」的话，「随时做」这一类任务
  /// 就再也建不出来了 —— 而它是 FR-TASK-01 里点了名的一档。
  /// 修 A 的时候顺手把 B 堵死，是这一晚上反复出现的那个形状。
  ///
  /// 时间轴一度是 true：那时它是**单日**刻度尺，确实对着某一天。
  /// 改成跨天议程之后这一项跟着翻面 —— 视图的形状变了，
  /// 它对「当前上下文」的答案也就变了。
