...
EXEMPT = {
    ...
    # ── M4：配置与提醒 ─────────────────────────────────────
    #
    # FR-NOTI-01/02/04 与 FR-CFG-05 已经落地并被用例点名，从表里去掉了。
    # 棘轮只许缩短 —— 这就是缩短的样子。
    'FR-NOTI-03': (
        'M4 机制已实现（manifest 的 boot receiver + 对账 + 进前台整窗口重排），'
        '但验收标准是「重启设备后待触发提醒不丢失」—— 那只能在真机上验，'
        '而真机那几项已按用户决定降级（notifications.md §11）'
    ),
    'NFR-REL-01': 'M4 未捕获异常落本地日志（bootstrap.dart 里的 TODO(M4)）',

    # ── M5：打磨与出包 ─────────────────────────────────────
    # FR-CFG-08 也去掉了：它的后半句「可通过导入的配置文件覆盖」
    # 需要一条真的导入路径，而 M4 把备份/恢复接上之后这条路走得通了
    # （hidden_settings_test 三句话各点一条）。
